...
class ReadOutModel(nn.Module):
    ...
    def __init__(self, model_dict, cuda):
        ...
        if architecture in ["LINEAR"]: # a model implemented in base_models.py
            self.rnn = constructor(model_dict)
        else: # a model implemented by pytorch
            self.rnn = constructor(input_size=model_dict['input_size'], hidden_size=model_dict['hidden_size'], num_layers=model_dict['num_layers'], batch_first=True)
