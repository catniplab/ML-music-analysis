...
def _initialize(model: ReadOutModel, architecture: str, initializer: dict) -> ReadOutModel:
    ...
    elif initializer['init'] == 'rnn':

        rnn_sd = torch.load(initializer['path'])

        in_shape = model.rnn.weight_ih_l0.data.shape
        model.rnn.weight_ih_l0.data = torch.zeros(in_shape)
        model.rnn.weight_ih_l0.data[2*in_shape[0]//3 : in_shape[0]] = rnn_sd['rnn.weight_ih_l0']
